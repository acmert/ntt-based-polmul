...
class NTT:
    ...
    def Radix2_DIT_Recursive_NTT(self,A,W,q):
        ...
        if N == 2:
            ...
            MulCnt = MulCnt+1 # This can be avoided in this and Iterative cases
            AddCnt = AddCnt+1
            SubCnt = SubCnt+1
            BtfCnt = BtfCnt+1
            ...
            BE,Mbe,Abe,Sbe,Bbe = self.Radix2_DIT_Recursive_NTT(AE,W_new,q)
            BO,Mbo,Abo,Sbo,Bbo = self.Radix2_DIT_Recursive_NTT(AO,W_new,q)
            MulCnt = MulCnt+Mbe+Mbo
            AddCnt = AddCnt+Abe+Abo
            SubCnt = SubCnt+Sbe+Sbo
            BtfCnt = BtfCnt+Bbe+Bbo
    ...
    def Radix2_DIF_Recursive_NTT(self,A,W,q):
        ...
        if N == 2:
            ...
            BE,Mbe,Abe,Sbe,Bbe = self.Radix2_DIF_Recursive_NTT(B[0:int(N/2)],W_new,q)
            BO,Mbo,Abo,Sbo,Bbo = self.Radix2_DIF_Recursive_NTT(B[int(N/2):N],W_new,q)
            MulCnt = MulCnt+Mbe+Mbo
            AddCnt = AddCnt+Abe+Abo
            SubCnt = SubCnt+Sbe+Sbo
            BtfCnt = BtfCnt+Bbe+Bbo
    ...
    def NaiveINTT_NN(self,A,W_inv,q):
        ...
        Very slow baseline implementation. Do not use for large parameter set.
        """
        P,MulCnt,AddCnt,SubCnt,BtfCnt = self.NaiveNTT_NN(A,W_inv,q)
        N_inv = modinv(len(A),q)
        B = [(x*N_inv) % q for x in P]
        MulCnt = MulCnt + len(P)
        return B,MulCnt,AddCnt,SubCnt,BtfCnt
    ...
    def NaiveINTT_NR(self,A,W_inv,q):
        ...
        Very slow baseline implementation. Do not use for large parameter set.
        """
        P,MulCnt,AddCnt,SubCnt,BtfCnt = self.NaiveNTT_NR(A,W_inv,q)
        N_inv = modinv(len(A),q)
        B = [(x*N_inv) % q for x in P]
        MulCnt = MulCnt + len(P)
        return B,MulCnt,AddCnt,SubCnt,BtfCnt
    ...
    def Radix2_DIT_Recursive_INTT(self,A,W_inv,q):
        P,MulCnt,AddCnt,SubCnt,BtfCnt = self.Radix2_DIT_Recursive_NTT(A,W_inv,q)
        N_inv = modinv(len(A),q)
        B = [(x*N_inv) % q for x in P]
        MulCnt = MulCnt + len(P)
        return B,MulCnt,AddCnt,SubCnt,BtfCnt
    ...
    def Radix2_DIF_Recursive_INTT(self,A,W_inv,q):
        P,MulCnt,AddCnt,SubCnt,BtfCnt = self.Radix2_DIF_Recursive_NTT(A,W_inv,q)
        N_inv = modinv(len(A),q)
        B = [(x*N_inv) % q for x in P]
        MulCnt = MulCnt + len(P)
        return B,MulCnt,AddCnt,SubCnt,BtfCnt
    ...
    def Radix2_DIT_Iterative_INTT_NR(self,A,W_inv,q):
        P,MulCnt,AddCnt,SubCnt,BtfCnt = self.Radix2_DIT_Iterative_NTT_NR(A,W_inv,q)
        N_inv = modinv(len(A),q)
        B = [(x*N_inv) % q for x in P]
        MulCnt = MulCnt + len(P)
        return B,MulCnt,AddCnt,SubCnt,BtfCnt
    ...
    def Radix2_DIT_Iterative_INTT_RN(self,A,W_inv,q):
        P,MulCnt,AddCnt,SubCnt,BtfCnt = self.Radix2_DIT_Iterative_NTT_RN(A,W_inv,q)
        N_inv = modinv(len(A),q)
        B = [(x*N_inv) % q for x in P]
        MulCnt = MulCnt + len(P)
        return B,MulCnt,AddCnt,SubCnt,BtfCnt
    ...
    def Radix2_DIT_Iterative_INTT_NN(self,A,W_inv,q):
        P,MulCnt,AddCnt,SubCnt,BtfCnt = self.Radix2_DIT_Iterative_NTT_NN(A,W_inv,q)
        N_inv = modinv(len(A),q)
        B = [(x*N_inv) % q for x in P]
        MulCnt = MulCnt + len(P)
        return B,MulCnt,AddCnt,SubCnt,BtfCnt
    ...
    def Radix2_DIF_Iterative_INTT_RN(self,A,W_inv,q):
        P,MulCnt,AddCnt,SubCnt,BtfCnt = self.Radix2_DIF_Iterative_NTT_RN(A,W_inv,q)
        N_inv = modinv(len(A),q)
        B = [(x*N_inv) % q for x in P]
        MulCnt = MulCnt + len(P)
        return B,MulCnt,AddCnt,SubCnt,BtfCnt
    ...
    def Radix2_DIF_Iterative_INTT_NN(self,A,W_inv,q):
        P,MulCnt,AddCnt,SubCnt,BtfCnt = self.Radix2_DIF_Iterative_NTT_NN(A,W_inv,q)
        N_inv = modinv(len(A),q)
        B = [(x*N_inv) % q for x in P]
        MulCnt = MulCnt + len(P)
        return B,MulCnt,AddCnt,SubCnt,BtfCnt
    ...
    def CRT_Recursive_NTT(self,A,W,q,pw=0,depth=1):
        ...
        if N == 1:
            ...
            # recall functions
            B[0:int(N/2)],Mbe,Abe,Sbe,Bbe = self.CRT_Recursive_NTT(B[0:int(N/2)], W,q,int(pw/2)                 ,2*depth)
            B[int(N/2):N],Mbo,Abo,Sbo,Bbo = self.CRT_Recursive_NTT(B[int(N/2):N], W,q,int(pw/2)+int((N/4)*depth),2*depth)
    ...
    def CRT_Recursive_INTT(self,A,W_inv,q):
        P,MulCnt,AddCnt,SubCnt,BtfCnt = self.CRT_Recursive_NTT(A,W_inv,q)
        N_inv = modinv(len(A),q)
        B = [(x*N_inv) % q for x in P]
        MulCnt = MulCnt + len(P)
        return B,MulCnt,AddCnt,SubCnt,BtfCnt
    ...
    def CRT_Full_INTT(self,A,W_inv,q):
        P,MulCnt,AddCnt,SubCnt,BtfCnt = self.CRT_Full_NTT(A,W_inv,q)
        N_inv = modinv(len(A),q)
        B = [(x*N_inv) % q for x in P]
        MulCnt = MulCnt + len(P)
        return B,MulCnt,AddCnt,SubCnt,BtfCnt
    ...
    def CooleyTukeyNTT(self,A,W,q):
        ...
        if (N == 2):
            ...
            B_even,Mbe,Abe,Sbe,Bbe = self.CooleyTukeyNTT(A_even,(W * W % q),q)
            B_odd ,Mbo,Abo,Sbo,Bbo = self.CooleyTukeyNTT(A_odd, (W * W % q),q)
    ...
    def FourStepNTT(self,A,W,q,size):
        ...
        if isinstance(size0, list):
            ...
            for i in range(int(N/temp0)):
                STEP_1[i],SM1,SA1,SS1,SB1 = self.FourStepNTT(STEP_1[i],W_0,q,size0)
            ...
            W_0 = (W ** int(N/temp0)) % q
            for i in range(int(N/temp0)):
                STEP_1[i],SM1,SA1,SS1,SB1 =  self.CooleyTukeyNTT(STEP_1[i],W_0,q)
        ...
        if isinstance(size1, list):
            ...
            for i in range(int(N/temp1)):
                STEP_3[i],SM3,SA3,SS3,SB3 = self.FourStepNTT(STEP_3[i],W_1,q,size1)
            ...
            W_1 = (W ** int(N/temp1)) % q
            for i in range(int(N/temp1)):
                STEP_3[i],SM3,SA3,SS3,SB3 = self.CooleyTukeyNTT(STEP_3[i],W_1,q)
        ...
        return STEP_4,MulCnt,AddCnt,SubCnt,BtfCnt
    ...
    def CooleyTukeyINTT(self,A,W_inv,q):
        P,MulCnt,AddCnt,SubCnt,BtfCnt = self.CooleyTukeyNTT(A,W_inv,q)
        N_inv = modinv(len(A),q)
        B = [(x*N_inv) % q for x in P]
        MulCnt = MulCnt + len(P)
        return B,MulCnt,AddCnt,SubCnt,BtfCnt
    ...
    def IterativeINTT(self,A,W_inv,q):
        P,MulCnt,AddCnt,SubCnt,BtfCnt = self.IterativeNTT(A,W_inv,q)
        N_inv = modinv(len(A),q)
        B = [(x*N_inv) % q for x in P]
        MulCnt = MulCnt + len(P)
        return B,MulCnt,AddCnt,SubCnt,BtfCnt
    ...
    def FourStepINTT(self,A,W_inv,q,size):
        P,MulCnt,AddCnt,SubCnt,BtfCnt = self.FourStepNTT(A,W_inv,q,size)
        N_inv = modinv(len(A),q)
        B = [(x*N_inv) % q for x in P]
        MulCnt = MulCnt + len(P)
        return B,MulCnt,AddCnt,SubCnt,BtfCnt
    ...
    def FourStepINTTv2(self,A,W_inv,q,size):
        P,MulCnt,AddCnt,SubCnt,BtfCnt = self.FourStepNTTv2(A,W_inv,q,size)
        N_inv = modinv(len(A),q)
        B = [(x*N_inv) % q for x in P]
        MulCnt = MulCnt + len(P)
        return B,MulCnt,AddCnt,SubCnt,BtfCnt
    ...
    def CTBased_ConstantGeometryINTT(self,A,W_inv,q):
        P,MulCnt,AddCnt,SubCnt,BtfCnt = self.CTBased_ConstantGeometryNTT(A,W_inv,q)
        N_inv = modinv(len(A),q)
        B = [(x*N_inv) % q for x in P]
        MulCnt = MulCnt + len(P)
        return B,MulCnt,AddCnt,SubCnt,BtfCnt
