...
class Poly:
    ...
    **** IMPORTANT NOTES HERE ***
    -- Our unified CRT-based function only performs CRT-based operations (not pure NTT/INTT operations!)
    ----- For findeg=1, it performs NTT+pre/pos-processing
    *******************************
    """
    ...
    def SchoolbookPolMul(self,A, B, q):
        ...
        for indexA, elemA in enumerate(A):
            for indexB, elemB in enumerate(B):
                ...
                MulCnt = MulCnt+1
                AddCnt = AddCnt+1
        return C,MulCnt,AddCnt,SubCnt,BtfCnt
        ...
        if len(A) < len(F):
            ...
            for i in range(len(D)-1,len(F)-2,-1):
                for j in range(len(R)):
                    ...
                    AddCnt = AddCnt+1
                D[i] = 0
            return D[0:len(F)-1],MulCnt,AddCnt,SubCnt,BtfCnt
    ...
    def SchoolbookModPolMul_PWC(self,A, B, q):
        ...
        return D,MulCnt,AddCnt,SubCnt,BtfCnt
    ...
    def SchoolbookModPolMul_NTRU(self,A, B, q):
        ...
        return D[0:len(A)],MulCnt,AddCnt,SubCnt,BtfCnt
    ...
    def NTTBasedPolMul(self,A,B,w,w_inv,q):
        ...
        A_ntt,MA,AA,SA,BA = Evaluator.Radix2_DIT_Iterative_NTT_NR(A_padded,w,q)
        B_ntt,MB,AB,SB,BB = Evaluator.Radix2_DIT_Iterative_NTT_NR(B_padded,w,q)
        MulCnt = MulCnt + MA + MB
        AddCnt = AddCnt + AA + AB
        SubCnt = SubCnt + SA + SB
        BtfCnt = BtfCnt + BA + BB
        ...
        C,MC,AC,SC,BC = Evaluator.Radix2_DIF_Iterative_INTT_RN(C_ntt,w_inv,q)
        MulCnt = MulCnt + MC
        AddCnt = AddCnt + AC
        SubCnt = SubCnt + SC
        BtfCnt = BtfCnt + BC
    ...
    def NTTBasedModPolMul_PWC(self,A,B,w,w_inv,q):
        ...
        A_ntt,MA,AA,SA,BA = Evaluator.Radix2_DIT_Iterative_NTT_NR(A,w,q)
        B_ntt,MB,AB,SB,BB = Evaluator.Radix2_DIT_Iterative_NTT_NR(B,w,q)
        MulCnt = MulCnt + MA + MB
        AddCnt = AddCnt + AA + AB
        SubCnt = SubCnt + SA + SB
        BtfCnt = BtfCnt + BA + BB
    ...
    def NTTBasedModPolMul_NWC_v1(self,A,B,w,w_inv,psi,psi_inv,q):
        ...
        MulCnt = MulCnt + len(A) + len(B)

        A_ntt,MA,AA,SA,BA = Evaluator.Radix2_DIT_Iterative_NTT_NR(A_p,w,q)
        B_ntt,MB,AB,SB,BB = Evaluator.Radix2_DIT_Iterative_NTT_NR(B_p,w,q)
        MulCnt = MulCnt + MA + MB
        AddCnt = AddCnt + AA + AB
        SubCnt = SubCnt + SA + SB
        BtfCnt = BtfCnt + BA + BB
        ...
        C_p,MC,AC,SC,BC = Evaluator.Radix2_DIF_Iterative_INTT_RN(C_ntt,w_inv,q)
        MulCnt = MulCnt + MC
        AddCnt = AddCnt + AC
        SubCnt = SubCnt + SC
        BtfCnt = BtfCnt + BC
        ...
        # There is no extra multiplication count since N_inv and psi_inv can be merged.
    ...
    def NTTBasedModPolMul_NWC_v2(self,A,B,psi,psi_inv,q):
        ...
        A_ntt,MA,AA,SA,BA = self.CTBasedMergedNTT_NR(A,psi,q)
        B_ntt,MB,AB,SB,BB = self.CTBasedMergedNTT_NR(B,psi,q)
        MulCnt = MulCnt + MA + MB
        AddCnt = AddCnt + AA + AB
        SubCnt = SubCnt + SA + SB
        BtfCnt = BtfCnt + BA + BB
        ...
        C,MC,AC,SC,BC = self.GSBasedMergedINTT_RN(C_ntt,psi_inv,q)
        MulCnt = MulCnt + MC
        AddCnt = AddCnt + AC
        SubCnt = SubCnt + SC
        BtfCnt = BtfCnt + BC
    ...
    def CRTBasedModPolMul_PWC(self,A,B,w,w_inv,q):
        ...
        A_ntt,MA,AA,SA,BA = Evaluator.CRT_Full_NTT(A,w,q)
        B_ntt,MB,AB,SB,BB = Evaluator.CRT_Full_NTT(B,w,q)
        MulCnt = MulCnt + MA + MB
        AddCnt = AddCnt + AA + AB
        SubCnt = SubCnt + SA + SB
        BtfCnt = BtfCnt + BA + BB
        ...
        C,MC,AC,SC,BC = Evaluator.CRT_Full_INTT(C_ntt,w_inv,q)
        MulCnt = MulCnt + MC
        AddCnt = AddCnt + AC
        SubCnt = SubCnt + SC
        BtfCnt = BtfCnt + BC
    ...
    def CRTBasedModPolMul_NWC_FD1(self,A,B,psi,psi_inv,q):
        ...
        A_ntt,MA,AA,SA,BA = self.CRT_Iterative_NWC_FD1(A,psi,q)
        B_ntt,MB,AB,SB,BB = self.CRT_Iterative_NWC_FD1(B,psi,q)
        MulCnt = MulCnt + MA + MB
        AddCnt = AddCnt + AA + AB
        SubCnt = SubCnt + SA + SB
        BtfCnt = BtfCnt + BA + BB
        ...
        C,MC,AC,SC,BC = self.ICRT_Iterative_NWC_FD1(C_ntt,psi_inv,q)
        MulCnt = MulCnt + MC
        AddCnt = AddCnt + AC
        SubCnt = SubCnt + SC
        BtfCnt = BtfCnt + BC
    ...
    def CRTBasedModPolMul_NWC_FD2(self,A,B,w,w_inv,q):
        ...
        A_ntt,MA,AA,SA,BA = self.CRT_Iterative_NWC_FD2_NR(A,w,q)
        B_ntt,MB,AB,SB,BB = self.CRT_Iterative_NWC_FD2_NR(B,w,q)
        MulCnt = MulCnt + MA + MB
        AddCnt = AddCnt + AA + AB
        SubCnt = SubCnt + SA + SB
        BtfCnt = BtfCnt + BA + BB
        ...
        for i in range(len(A)//2):
            ...
            wk    = pow(w,w_pow,q)
            C_ntt[2*i:2*i+2],MP,AP = self.PolWiseMult(A_ntt[2*i:2*i+2],B_ntt[2*i:2*i+2],wk,2,q)
            MulCnt = MulCnt + MP
            AddCnt = AddCnt + AP

        C,MC,AC,SC,BC = self.ICRT_Iterative_NWC_FD2_RN(C_ntt,w_inv,q)
        MulCnt = MulCnt + MC
        AddCnt = AddCnt + AC
        SubCnt = SubCnt + SC
        BtfCnt = BtfCnt + BC
    ...
    def CRTBasedModPolMul_NWC_FDV(self,A,B,w,w_inv,q,findeg):
        ...
        A_ntt,MA,AA,SA,BA = self.CRT_Iterative_NWC_FDV_NR(A,w,q,findeg)
        B_ntt,MB,AB,SB,BB = self.CRT_Iterative_NWC_FDV_NR(B,w,q,findeg)
        MulCnt = MulCnt + MA + MB
        AddCnt = AddCnt + AA + AB
        SubCnt = SubCnt + SA + SB
        BtfCnt = BtfCnt + BA + BB
        ...
        for i in range(len(A)//findeg):
            ...
            wk    = pow(w,w_pow,q)
            C_ntt[findeg*i:findeg*i+findeg],MP,AP = self.PolWiseMult(A_ntt[findeg*i:findeg*i+findeg],B_ntt[findeg*i:findeg*i+findeg],wk,findeg,q)
            MulCnt = MulCnt + MP
            AddCnt = AddCnt + AP

        C,MC,AC,SC,BC = self.ICRT_Iterative_NWC_FDV_RN(C_ntt,w_inv,q,findeg)
        MulCnt = MulCnt + MC
        AddCnt = AddCnt + AC
        SubCnt = SubCnt + SC
        BtfCnt = BtfCnt + BC
    ...
    def CRTBasedModPolMul_NTRU_FD3(self,A,B,w,w_inv,ntrupowersf,ntrupowersb,ntrupowersi,q):
        ...
        # NTT
        A_ntt,MA,AA,SA,BA = self.CRT_Iterative_NTRU_FD3_NR(A_r,w,ntrupowersf,q)
        B_ntt,MB,AB,SB,BB = self.CRT_Iterative_NTRU_FD3_NR(B_r,w,ntrupowersf,q)
        MulCnt = MulCnt + MA + MB
        AddCnt = AddCnt + AA + AB
        SubCnt = SubCnt + SA + SB
        BtfCnt = BtfCnt + BA + BB
        ...
        for i in range(len(A)//3):
            w_pow = ntrupowersb[i]
            wk    = pow(w,w_pow,q)
            C_ntt[3*i:3*i+3],MP,AP = self.PolWiseMult(A_ntt[3*i:3*i+3],B_ntt[3*i:3*i+3],wk,3,q)
            MulCnt = MulCnt + MP
            AddCnt = AddCnt + AP

        # INTT
        C,MC,AC,SC,BC = self.ICRT_Iterative_NTRU_FD3_RN(C_ntt,w_inv,ntrupowersi,q)
        MulCnt = MulCnt + MC
        AddCnt = AddCnt + AC
        SubCnt = SubCnt + SC
        BtfCnt = BtfCnt + BC
        ...
        return C_r,MulCnt,AddCnt,SubCnt,BtfCnt
    ...
    def CRTBasedModPolMul_NTRU_FDV(self,A,B,w,w_inv,ntrupowersf,ntrupowersb,ntrupowersi,q,findeg):
        ...
        # NTT
        A_ntt,MA,AA,SA,BA = self.CRT_Iterative_NTRU_FDV_NR(A_r,w,ntrupowersf,q,findeg)
        B_ntt,MB,AB,SB,BB = self.CRT_Iterative_NTRU_FDV_NR(B_r,w,ntrupowersf,q,findeg)
        MulCnt = MulCnt + MA + MB
        AddCnt = AddCnt + AA + AB
        SubCnt = SubCnt + SA + SB
        BtfCnt = BtfCnt + BA + BB
        ...
        # INTT
        C,MC,AC,SC,BC = self.ICRT_Iterative_NTRU_FDV_RN(C_ntt,w_inv,ntrupowersi,q,findeg)
        MulCnt = MulCnt + MC
        AddCnt = AddCnt + AC
        SubCnt = SubCnt + SC
        BtfCnt = BtfCnt + BC
    ...
    def CRTBasedModPolMul_Unified(self,A,B,w,w_inv,q,ring,findeg,ntrupowersf=[],ntrupowersb=[],ntrupowersi=[]):
        ...
        # --------------------------------------------- NTT
        A_ntt,MA,AA,SA,BA = self.CRT_Iterative_Unified_NR(A_r,w,q,ring,findeg,ntrupowersf)
        B_ntt,MB,AB,SB,BB = self.CRT_Iterative_Unified_NR(B_r,w,q,ring,findeg,ntrupowersf)
        MulCnt = MulCnt + MA + MB
        AddCnt = AddCnt + AA + AB
        SubCnt = SubCnt + SA + SB
        BtfCnt = BtfCnt + BA + BB
        ...
        # --------------------------------------------- INTT
        C,MC,AC,SC,BC = self.ICRT_Iterative_Unified_RN(C_ntt,w_inv,q,ring,findeg,ntrupowersi)
        MulCnt = MulCnt + MC
        AddCnt = AddCnt + AC
        SubCnt = SubCnt + SC
        BtfCnt = BtfCnt + BC
    ...
    def CTBasedMergedNTT_NR(self,A,Psi,q):
        ...
        return B,MulCnt,AddCnt,SubCnt,BtfCnt
